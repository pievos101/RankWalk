...
# --------------------------
# Jaccard-anchored random walk pairs (LOCAL only)
# --------------------------
def jaccard_walk_pairs(
    J,
    edge_index,
    walk_length=6,
    num_walks=10,
        ...
        for _ in range(num_walks):
            ...
            for _ in range(walk_length):
                ...
                next_node = random.choices(
                    nbrs, weights=probs.cpu().tolist(), k=1
                )[0]

                walk.append(next_node)
                cur = next_node

            # LOCAL co-occurrence only (prevents long-walk degradation)
            for i in range(len(walk)):
                for j in range(i + 1, min(i + 1 + max_hops, len(walk))):
                    pos_pairs.append((walk[i], walk[j]))

    return pos_pairs

# --------------------------
# Contrastive loss on walk co-occurrences
# --------------------------
...
# Training
# --------------------------
def train(model, x, edge_index, epochs=800, lr=0.05):
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    best_loss = float("inf")

    # Precompute walk pairs ONCE
