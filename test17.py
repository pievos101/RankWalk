...
class GNN(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        return h2  # final embeddings

model = GNN(x.size(1), 32).to(device)

# Contrastive-style loss
def contrastive_loss(emb, edge_index):
    src, dst = edge_index
    pos_loss = F.mse_loss(emb[src], emb[dst])  # neighbors close
    
    # Sample negatives
    num_nodes = emb.size(0)
    neg_i = torch.randint(0, num_nodes, (len(src),), device=device)
    neg_j = torch.randint(0, num_nodes, (len(dst),), device=device)
    neg_dist = torch.norm(emb[neg_i] - emb[neg_j], dim=1)
    neg_loss = F.relu(0.5 - neg_dist).mean()  # push non-neighbors apart
    
    return pos_loss + neg_loss

# Training
def train(model, x, edge_index, epochs=1000, lr=0.01):
