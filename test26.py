...
# --------------------------
# Jaccard-anchored random walks
# --------------------------
def jaccard_walk_pairs(J, edge_index, walk_length=3, num_walks=10, eps=1e-6):
    neighbors = [[] for _ in range(num_nodes)]
    for u, v in edge_index.t().tolist():
        neighbors[u].append(v)
        ...
        for _ in range(num_walks):
            ...
            for _ in range(walk_length):
                ...
                if not nbrs:
                    break
                probs = torch.tensor(
                    [J[start, n].item() + eps for n in nbrs], device=device
                )
                probs /= probs.sum()
                cur = random.choices(nbrs, weights=probs.cpu().tolist(), k=1)[0]
                walk.append(cur)
...
# --------------------------
# Stabilized contrastive loss
# --------------------------
def contrastive_loss_walks(
    emb,
    pos_pairs,
    neg_multiplier=2,
    temperature=0.2
    ...
    pos_loss = ((1 - pos_sim).clamp(min=0.0) * pos_w).mean()
...
# Training
# --------------------------
def train(model, x, edge_index, epochs=800, lr=0.005):
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    best_loss = float("inf")

    pos_pairs = jaccard_walk_pairs(
        J,
        edge_index,
        walk_length=10,
        num_walks=10
...
def evaluate(emb, labels):
    ...
    print(
        f"-> KMeans ARI: {adjusted_rand_score(labels, km.labels_):.3f}, "
        f"NMI: {normalized_mutual_info_score(labels, km.labels_):.3f}"
    )
    print(
        f"-> Ward   ARI: {adjusted_rand_score(labels, ward.labels_):.3f}, "
