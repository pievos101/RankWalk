...
# --------------------------
# Start-node anchored positive pairs
# --------------------------
def sample_pos_pairs_start_anchor(J, edge_index, num_nodes, walk_length=10, top_k=5, eps=1e-6):
    """
    Generate positive pairs anchored at start node.
    Each node defines the anchor; no repeated walks needed.
    """
    ...
    for start in range(num_nodes):
        ...
        for step in range(1, walk_length + 1):
            ...
            if nbrs.numel() == 0:
                break
            probs = J[start, nbrs] + eps        # start-node anchored
            probs /= probs.sum()
            cur = nbrs[torch.multinomial(probs, 1).item()]
            if cur not in visited:
                visited[cur] = step + 1

        # Borda-like ranking: top-k neighbors
...
def run_node2vec(G, dim=48):
    node2vec = Node2Vec(G, dimensions=dim, walk_length=30, num_walks=100, p=1, q=1, workers=1, seed=42)
...
if __name__ == "__main__":
    ...
    G_nx, labels = generate_sbm_graph(n_communities=3, size_per_comm=40, p_in=0.2, p_out=0.05)
    ...
    # Train start-node anchored GNN
    emb_gnn = train_gnn(x, edge_index, J, epochs=500, walk_length=30, top_k=30, device=device)
    evaluate(emb_gnn, labels, "StartAnchor GNN (contrastive)")
