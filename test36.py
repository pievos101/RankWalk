...
# --------------------------
# GNN with Jaccard-weighted message passing
# --------------------------
...
def sample_ranked_pos_pairs(J, edge_index, num_nodes, walk_length=10, num_walks=10, top_k=5, eps=1e-6):
    ...
    for start in range(num_nodes):
        ...
        # multiple random walks per anchor
        ...
        # take top_k closest nodes
        topk = sorted(borda, key=borda.get)[:top_k]
        max_rank = max([borda[v] for v in topk])

        # assign weight proportional to closeness: lower rank => higher weight
        for v in topk:
            weight = 1.0 - (borda[v] / (max_rank + 1e-6))  # normalize [0,1]
            if v != start:
...
def train_gnn(x, edge_index, J, epochs=500, lr=1e-3, walk_length=10, num_walks=10, top_k=5):
    model = JaccardGNN(x.size(1), out_dim=48)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)

    # Prepare edge weights from Jaccard
    edge_weight = torch.tensor([J[u, v] for u, v in edge_index.t().tolist()])
...
if __name__ == "__main__":
    ...
    # Node features: random vectors (can be improved with structural features)
    x = torch.randn(G_nx.number_of_nodes(), 20)

    # Precompute Jaccard for edge weighting
    num_nodes = G_nx.number_of_nodes()
    J = compute_jaccard(edge_index, num_nodes)

    # Train GNN end-to-end with weighted positives
    emb_gnn = train_gnn(x, edge_index, J, epochs=500, walk_length=10, num_walks=10, top_k=5)
