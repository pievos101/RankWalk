...
# --------------------------
# Sample positives dynamically (TopKGraphs style)
# --------------------------
def sample_pos_pairs(J, edge_index, num_nodes, walk_length=10, num_walks=10, top_k=5, eps=1e-6):
    neighbors = [[] for _ in range(num_nodes)]
    for u, v in edge_index.t().tolist():
        neighbors[u].append(v)
    pos_pairs = []
...
# --------------------------
def contrastive_loss(emb, pos_pairs, temperature=0.2, neg_multiplier=2):
    device = emb.device
    ...
    # negatives: sample for each positive
    neg_exp_list = []
    for pi in pos_i:
    ...
    neg_exp_sum = torch.stack(neg_exp_list)  # same size as pos_exp
...
def train_gnn(x, edge_index, J, epochs=500, lr=1e-3, walk_length=10, num_walks=10, top_k=5):
    ...
    for epoch in range(1, epochs + 1):
        optimizer.zero_grad()
        emb = model(x, edge_index, edge_weight=None)  # edge_weight optional
...
if __name__ == "__main__":
    ...
    # Precompute Jaccard for sampling
    num_nodes = G_nx.number_of_nodes()
    J = compute_jaccard(edge_index, num_nodes)

    # Train GNN end-to-end with TopKGraphs-style contrastive positives
    emb_gnn = train_gnn(x, edge_index, J, epochs=1000, walk_length=20, num_walks=10, top_k=3)
